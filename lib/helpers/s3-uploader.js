...
module.exports = class S3Uploader {
	 ...
	 * @throws {SnsTriggerError} - Throws an error if the upload fails for all provided buckets.
	 */
	static async uploadContentS3Path(buckets, contentS3Path, body) {
		...
		throw new SnsTriggerError('Failed to upload to all provided buckets', SnsTriggerError.codes.S3_ERROR);
	}
