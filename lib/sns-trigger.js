...
module.exports = class SnsTrigger {
	...
	async publishEvent(topicArn, event) {
		...
		if(parsedEvent.limitExceeded) {
			...
			await S3Uploader.uploadS3ContentPath(bucketList, s3ContentPath, parsedEvent.Message);
	...
	async publishEvents(topicArn, events) {
		...
		const asyncWithConcurrency = new AsyncWithConcurrency(async batch => {
			...
			for(const { limitExceeded, extraProperties: { payloadFixedProperties, s3ContentPath }, ...parsedEvent } of batch) {
				...
				promises.push(S3Uploader.uploadS3ContentPath(bucketList, s3ContentPath, parsedEvent.Message));
			}
